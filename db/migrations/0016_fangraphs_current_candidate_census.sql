DROP MATERIALIZED VIEW app.fangraphs_current_scouting_snapshot;
...
SELECT
  ...
  coalesce(
    app.jsonb_number(joined.scouting_payload, 'Age'),
    app.jsonb_number(joined.statistics_payload, 'Age')
  ) AS age,
  coalesce(
    nullif(joined.scouting_payload ->> 'UPURL', ''),
    nullif(joined.statistics_payload ->> 'UPURL', '')
  ) AS fangraphs_path,
  joined.report_season,
  app.jsonb_number(joined.statistics_payload, 'Season')::integer AS stats_season,
...
COMMENT ON MATERIALIZED VIEW app.fangraphs_current_scouting_snapshot IS
  'Latest validated current-season FanGraphs scouting and completed-stat context per exact player-role UPID and MinorMaster identity. MLBAM is joined only when the same response carries an exact UPID+MinorMaster pair; names are display fields and never identity keys. Exact MLBAM rows may seed the active prospect candidate census.';
