...
components AS (
  SELECT
    ...
    coalesce(app.jsonb_number(batting.payload, 'b_pa'), 0) >= 25 AS has_substantive_batting,
    coalesce(app.jsonb_number(pitching.payload, 'p_ip_outs'), 0) >= 30 AS has_substantive_pitching,
...
classified AS (
  SELECT
    components.*,
    CASE
      WHEN has_substantive_batting AND has_substantive_pitching THEN 'Two-way'
      WHEN coalesce(p_ip_outs, 0) > 0 AND coalesce(b_pa, 0) <= 10 THEN 'Pitcher'
      ELSE 'Hitter'
...
COMMENT ON MATERIALIZED VIEW app.current_mlb_value_snapshot IS
  'Latest complete Baseball-Reference current-value pair. Two-way comparison requires at least 25 PA and 10 IP; nominal cross-page appearances are retained as components but do not define role. Percentiles require at least 25 peers.';
